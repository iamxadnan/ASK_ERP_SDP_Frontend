...
export default function Facultyhome() {
  ...
  useEffect(() => {
    ...
    if (email) {
      ...
      fetch(`${config.url}/faculty/profile-picture?email=${email}`)
        .then((response) => {
          if (response.ok) {
  ...
  useEffect(() => { 
    if (facultyId) { 
        ...
        fetch(`${config.url}/faculty/courses/${facultyId}`) 
            .then((response) => { 
    ...
    <div className="student-home">
      ...
      <div className="header-banner">
        <div className="welcome-message">
          <p className="date">{new Date().toLocaleDateString()}</p> {/* Display today's date */}
          <h1>Welcome back, {firstname}!</h1>
          <p>Always stay updated on our ERP</p>
        </div>
